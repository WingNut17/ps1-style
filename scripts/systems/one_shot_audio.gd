## A oneshot audio that will automatically handle random wait times and pitch scales.
...
func _ready() -> void:
	...
	self.pitch_scale = randf_range(random_pitch.min(), random_pitch.max())
