...
## Used for animating the dialogue open and close
func animate_dialogue_menu(state: String):
	...
	match state:
		...
		"close":
			...
			tween.parallel().tween_property(responses_menu, "scale", Vector2(1,0), 0.75).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUART)


## Used for queueing the dialogue free when its over
func _end_close_tween() -> void:
	queue_free()
